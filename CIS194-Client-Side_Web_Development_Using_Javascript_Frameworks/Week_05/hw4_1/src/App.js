...
const App = () => {
  ...
  const handleSearch = event =>{
    // Update searchToc to be what the user inputs in the seach field
  ...
  // Filter toc to be only items with a title that match the given search
  const searchResults = toc.filter(item=> item.title.search(searchToC) >= 0)
